...
u16 SampleFrequency[12] = { 33452, 35441, 37549, 39781, 42147, 44653, 47308, 50121, 53102, 56259, 59605, 63149 };
...
u16 sinus[17] = { 0, 100, 200, 297, 392, 483, 569, 650, 724, 792, 851, 903, 946, 980, 1004, 1019, 1024 };
...
void XM7_lowlevel_startSoundwLoop(int sampleRate, const void *data, u32 looplength, u32 loopstart, u8 channel, u8 vol, u8 pan, u8 format, u32 offset)
{
    ...
    if ((loopstart + looplength) > offset)
    {
        ...
        if (offset > loopstart) offset = (format ? (loopstart << 1) : loopstart);
...
// define this to access the higher byte of SCHANNEL_CR
...
void SlideSampleVolume(XM7_ModuleManager_Type *module, u8 chn, s8 change)
{
    ...
    if (change > 0)
    {
        ...
        if (module->CurrentSampleVolume[chn] > 0x40) module->CurrentSampleVolume[chn] = 0x40;
    }
    else
    {
...
u16 DecodeVolumeColumn(XM7_ModuleManager_Type *module, u8 chn, u8 volcmd, u8 curtick, u8 EDxInAction)
{
    ...
    switch (volcmd)
    {
        ...
        case 0x10 ... 0x50:
            ...
            if (curtick == EDxInAction) module->CurrentSampleVolume[chn] = (volcmd - 0x10);   // 0..0x40
            break;

        case 0x61 ... 0x6f:
            // VOLUME slide down (decrease volume by value)
            // NOTE: the effect starts from tick=1, not on tick=0
            if (curtick != 0) SlideSampleVolume(module, chn, -tmpvalue);
            break;
        ...
        case 0xa0 ... 0xaf:
            ...
            {
                // memory effect
                if (tmpvalue != 0) module->Effect4xxMemory[chn] = (module->Effect4xxMemory[chn] & 0x0f) | (tmpvalue << 4);
                resvalue = 0x0000;
            }
            else
            {
        ...
        case 0xb0 ... 0xbf:
            ...
            {
                // memory effect
                if (tmpvalue != 0) module->Effect4xxMemory[chn] = (module->Effect4xxMemory[chn] & 0xf0) | tmpvalue;
                resvalue = 0x0000;
            }
            else
            {
        ...
        case 0xc0 ... 0xcf:
            // change panning
            if (curtick == 0) module->CurrentSamplePanning[chn] = (tmpvalue << 4) | tmpvalue; // 0..0xff
            break;

        case 0xd1 ... 0xdf:
            // PANNING slide LEFT
            // NOTE: the effect starts from tick=1, not on tick=0
            if (curtick > 0) SlideSamplePan(module, chn, -tmpvalue);
            break;
        ...
        case 0xf0 ... 0xff:
            ...
            else if (curtick == 0) module->Effect3xxMemory[chn] = tmpvalue;
...
u16 DecodeBeforeEffectsColumn(u8 chn, u8 effcmd, u8 effpar, u8 curtick)
{
    ...
    switch (effcmd)
    {
        ...
        case 0x14:                                                        // Kxx
            if (effpar == curtick) resvalue = 0x1400;
            break;
...
u16 DecodeEffectsColumn(XM7_ModuleManager_Type *module, u8 chn, u8 effcmd, u8 effpar, u8 curtick, u16 addtick)
{
    ...
    switch (effcmd)
    {
        ...
        case 0x8:
            ...
            if (curtick == 0) module->CurrentSamplePanning[chn] = effpar;
            break;
        ...
        case 0x9:
            ...
            if (curtick == 0) effpar = MemoryEffectxxTogether(effpar, &module->Effect9xxMemory[chn]);
        ...
        case 0xc:
            ...
            if (curtick == 0)
            {
                if (effpar <= 0x40) module->CurrentSampleVolume[chn] = effpar;
            }
            break;
        ...
        case 0xe:
            ...
            switch (effpar >> 4)
            {
                ...
                case 0x3:
                    ...
                    if (curtick == 0)
                    {
                        if (tmpvalue < 2) module->CurrentGlissandoType[chn] = tmpvalue;
                    }
                    break;
                ...
                case 0x4:
                    ...
                    if (curtick == 0)
                    {
                        ...
                        else if (tmpvalue == 7) module->CurrentVibratoType[chn] = rand() % 3 + 4; // set to 4,5,6
                    }
                    break;
                ...
                case 0x6:
                    ...
                    {
                        if (curtick == 0) module->CurrentLoopBegin[chn] = module->CurrentLine;
                    }
                    else
                    {
                ...
                case 0x7:
                    ...
                    if (curtick == 0)
                    {
                        ...
                        else if (tmpvalue == 7) module->CurrentTremoloType[chn] = rand() % 3 + 4; // set to 4,5,6
                    }
                    break;
                ...
                case 0x9:
                    ...
                    if (curtick != 0)
                    {
                        if ((curtick % tmpvalue) == 0)
                            ...
                            resvalue = 0xe90;  // gives back 1 if retrig needed
                    }
                    break;
                ...
                case 0xc:
                    ...
                    if (curtick == tmpvalue) module->CurrentSampleVolume[chn] = 0;
                    break;

                case 0xe:
                    // "Pattern delay": Delays playback progression for the duration of x rows
                    if (curtick == 0) module->CurrentDelayLines = tmpvalue;
                    break;
            }
            break;
        ...
        case 0x11:                                  // Hxy
            ...
            if (curtick > 0)
            {
                ...
                if (tmpvalue != 0)
                {
                    ...
                    if (module->CurrentGlobalVolume > 0x40) module->CurrentGlobalVolume = 0x40;
                }
                else
                {
                    tmpvalue = (effpar & 0x0F);                   // volume DOWN
        ...
        case 0x1b:                                                        // Rxy
            ...
            if (curtick != 0)
            {
                ...
                if (tmpvalue != 0)
                {
                    ...
                    if ((curtick % tmpvalue) == 0) resvalue = 0x1b01;      // gives back 1 if retrig needed
                }
...
s8 CalculateAutoVibrato(XM7_ModuleManager_Type *module, u8 chn, u8 instrument)
{
    ...
    switch (instr->VibratoType)
    {
        ...
        case 1:
            ...
            if (module->CurrentAutoVibratoPoint[chn] > 127) autovib = -autovib;
            break;
